pipeline {
    ...
    environment {
        JMETER_HOME_WIN = "C:/Users/USER/Downloads/QA부트캠프/QA용SW프로그램/실행용/apache-jmeter-5.6.3"
    }
        ...
        stage('Checkout JMX from GitHub') {
            steps {
                git branch: 'main',
                    ...
                    url: 'https://github.com/nwweiit/test-jenkins-jmeter.git'
            }
        }
        ...
        stage('Run Performance Test') {
            steps {
                script {
                    ...
                    // ⭐ JMeter 실행 한 줄로 고정
                    bat """
        ${jmeterCmd} -n -t "${env.JMX_FILE}" -l results.jtl -e -o reports
        """
                }
            }
        }
